  ...
  return (
    <section id="home" className="min-app-screen relative flex items-center justify-center overflow-hidden pt-20">
      ...
      <div className="relative z-10 container mx-auto px-4 text-center">
        <h1 className="mb-5 font-chinese text-5xl font-bold leading-tight text-gray-800 md:mb-6 md:text-7xl">
          <span className="text-amber-700 md:bg-clip-text md:text-transparent md:bg-gradient-to-r md:from-amber-600 md:to-orange-500">
            {convert('佛法如燈')}
          </span>
          <br />
          <span className="text-[2.5rem] md:text-5xl">{convert('照破千年暗')}</span>
        </h1>

        <p className="mx-auto mb-10 max-w-2xl font-chinese text-xl leading-relaxed text-gray-600 md:mb-12 md:text-2xl">
        ...
        <div className="mx-auto mt-14 grid max-w-2xl grid-cols-3 gap-4 md:mt-16 md:gap-8">
          <div className="text-center">
            <div className="mb-2 text-5xl font-bold text-amber-600 md:text-5xl">{articleCount}</div>
            <div className="text-lg text-gray-600 md:text-base">{convert('文章總數')}</div>
          </div>
          <div className="text-center">
            <div className="mb-2 text-5xl font-bold text-amber-600 md:text-5xl">{teacherCount}</div>
            <div className="text-lg text-gray-600 md:text-base">{convert('法師開示')}</div>
          </div>
          <div className="text-center">
            <div className="mb-2 text-5xl font-bold text-amber-600 md:text-5xl">{categoryCount}</div>
            <div className="text-lg text-gray-600 md:text-base">{convert('法門分類')}</div>
          </div>
        </div>
      </div>
